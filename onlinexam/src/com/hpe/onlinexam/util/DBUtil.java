...
public class DBUtil {
	...
	public List<Class<?>> getQueryList(Class<?> type, String sql, Object[] paramList) throws Exception {
		BeanInfo beanInfo = Introspector.getBeanInfo(type);//获得type类的所有属性方法信息
		PropertyDescriptor[] propertyDescriptors = beanInfo.getPropertyDescriptors();//获得所有属性描述
		List<Map<String, Object>> list = getQueryList(sql, paramList);
		List beanList = new ArrayList();
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			Map<String, Object> map = (Map<String, Object>) iterator.next();
			Object obj = type.newInstance();//创建一个type类的对象
			for (int i = 0; i < propertyDescriptors.length; i++) {
				PropertyDescriptor descriptor = propertyDescriptors[i];
				String propertyName = descriptor.getName();//得到属性的名称
				if (map != null && map.containsKey(propertyName)) {
					Object value = map.get(propertyName);
					try {
						descriptor.getWriteMethod().invoke(obj, value);//执行写入属性值的方法，将value写入obj对象中
					} catch (Exception e) {
						logger.info("检测一下Table列，和实体类属性：" +
					propertyName + "" + "是否一致，并且是否是" + value.getClass() + "类型");
						throw new Exception(
								"检测一下Table列，和实体类属性：" + 
					propertyName + "" + "是否一致，并且是否是" + value.getClass() + "类型");
					}
